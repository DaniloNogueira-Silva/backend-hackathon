...
export class AgendamentoChatService {
  ...
  async processarAgendamento(
    ...
    switch (currentState.step) {
      ...
      case AgendamentoStep.AGUARDANDO_CONFIRMACAO:
        resposta = await this.handleConfirmacaoStep(
          currentState,
          pergunta,
          sessionId,
        );
      ...
      default:
        resposta =
          'Não consigo te ajudar com essa pergunta. Sou um assistente focado em telemedicina. Por favor, digite uma pergunta mais clara.';
  ...
  private async handleEspecialidadeStep(
    state: ConversationState,
    especialidade: string,
  ): Promise<string> {
    console.log('state', state, 'especialidade', especialidade);
    const medicos =
      await this.medicosService.findByEspecialidade(especialidade);
  ...
  private async handleMedicoStep(
    ...
    if (!isNaN(index) && medicos[index]) {
      ...
      medicoEscolhido = medicos.find((m) =>
        m.nome.toLowerCase().includes(escolha.toLowerCase()),
      );
    }
  ...
  private async handleHorarioStep(
    state: ConversationState,
    horario: string,
  ): Promise<string> {
...
Tudo certo? (Responda "sim" para confirmar)`;
  ...
  private async handleConfirmacaoStep(
    state: ConversationState,
    confirmacao: string,
    sessionId: string,
  ): Promise<string> {
